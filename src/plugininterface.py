...
class plugininterface(baseClass):
        ...
        for dir in os.listdir(self.plugin_dir):
            ...
            if (os.path.isdir(dirpath) and dir[0] != "."):
                ...
                if (os.path.exists(pluginxml)):
                    ...
                    self._log("Trying to load plugin '%s'..." %
                              dir, self.LOG_DEBUG)
                    loadedClass = None
                    ...
                    self._log(
                        "Plugin doesn't have a plugin.xml file! -> '%s'..." % dir, self.LOG_WARN)
    ...
    def requestPluginActions(self, langClass, isSystem, isUnix):
        ret = []
        for p in self.plugins:
            modes = p.plugin_exploit_modes_requested(
                langClass, isSystem, isUnix)
            for m in modes:
    ...
    def requestPluginFallbackActions(self, langClass):
        ...
        for p in self.plugins:
            try:
                ...
                # Compatiblity for old plugins cause I am currently too lazy to fix them.
                pass
        return(ret)
    ...
    def broadcast_callback(self, attack, haxhelper):
        for p in self.plugins:
            try:
                ...
                self._log("\nPlugin '%s' just crashed!" %
                          (p.getPluginName()), self.LOG_ERROR)
                self._log("Please send a bugreport to the Plugin Developer: %s <%s>" % (
                    p.getPluginAutor(), p.getPluginEmail()), self.LOG_ERROR)
...
class basePlugin(baseClass):
    ...
    def printInfo(self):
        self._log("[%s version %d]" % (self.getPluginName(),
                                       self.getPluginVersion()), self.LOG_DEBUG)
