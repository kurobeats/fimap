...
class googleScan:
    ...
    def __init__(self, config):
        ...
        self.gs = GoogleSearch(
            self.config["p_query"], page=self.config["p_skippages"], random_agent=True)
        ...
        if (self.config["p_skippages"] > 0):
            print("Google Scanner will skip the first %d pages..." %
    ...
    def startGoogleScan(self):
        print("Querying Google Search: '%s' with max pages %d..." %
            ...
            while (redo):
                try:
                    ...
                    if (diff <= self.cooldown):
                        if (diff > 0):
                            print("Commencing %ds google cooldown..." %
                                  (self.cooldown - diff))
