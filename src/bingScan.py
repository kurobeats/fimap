...
# TODO https://github.com/tristantao/py-ms-cognitive#usage
...
class bingScan:
    ...
    def __init__(self, config):
        self.config = config
        self.bs = PyMsCognitiveWebSearch('API_KEY', search_term)
        ...
        while(pagecnt < self.config["p_pages"]):
            ...
            while (redo):
                try:
                    ...
                    last_request_time = datetime.datetime.now()
                    resp = self.bs.search_web(self.config["p_query"], {
                                              'Web.Count': 50, 'Web.Offset': (pagecnt-1)*self.results_per_page})
