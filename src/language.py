...
class XML2Config(baseClass):
    ...
    def __init_xmlresult(self):
        ...
        if (os.path.exists(xmlfile)):
            ...
            self._log("Mindepth (%d) and Maxdepth (%d) loaded from generic.xml." % (
                mindepth, maxdepth), self.LOG_DEBUG)
            ...
            self.commandConcat_unix = str(
                unix_node.getAttribute("concatcommand"))

            quiz_node = getXMLNode(unix_node, "shellquiz")
            self.shellquiz_code_unix = base64.b64decode(
                quiz_node.getAttribute("source"))
            kernel_node = getXMLNode(unix_node, "kernelversion")
            self.kernelversion_code_unix = str(
                kernel_node.getAttribute("source"))
    ...
    def __loadLanguageSets(self):
        ...
        for c in langnodes.childNodes:
            if (c.nodeName == "language"):
                ...
                self._log("Loaded XML-LD for '%s' at revision %d by %s" % (langname,
                                                                           langClass.getRevision(), langClass.getAutor()), self.LOG_DEBUG)
...
class baseLanguage(baseTools):
    ...
    def generateQuiz(self):
        ...
        try:
            ...
            boxarr.append(
                "The quiz function defined in one of the XML-Language-Definition files")
            boxarr.append(
                "just failed! If you are coding your own XML then fix that!")
            boxarr.append(
                "If not please report this bug at http://fimap.googlecode.com (!) Thanks!")
    ...
    def __populate(self):
        ...
        self.do_force_inclusion_test = self.XML_Rootitem.getAttribute(
            "force_inclusion_test") == "1"
        ...
        if (len(self.exec_methods) == 0):
            ...
            self._log(
                "  This XML-LD can't be used to go into exploit mode!", self.LOG_ERROR)
        ...
        self.sniper_regex = str(getXMLNode(
            self.XML_Rootitem, "snipe").getAttribute("regex"))
        if (self.sniper_regex == None or self.sniper_regex.strip() == ""):
            self._log(
                "XML-LD has no sniper regex! So this XML-LD can only be used in blind-mode!", self.LOG_WARN)

        methods_node = getXMLNode(self.XML_Rootitem, "methods")
        quiz_node = getXMLNode(methods_node, "quiz")
        if (quiz_node == None):
            self._log("FATAL! XML-Language-Definition (%s) has no quiz function defined!" %
                      (self.getName()), self.LOG_ERROR)
            self._log(
                "Please fix that in order to run fimap without problems!", self.LOG_ERROR)
        ...
        if (print_node == None):
            self._log("FATAL! XML-Language-Definition (%s) has no print function defined!" %
                      (self.getName()), self.LOG_ERROR)
            self._log(
                "Please fix that in order to run fimap without problems!", self.LOG_ERROR)
            self._log("Committing suicide :-O", self.LOG_ERROR)
        ...
        if (eval_node == None):
            self._log("XML-LD (%s) has no eval_kickstarter method defined." %
                      (self.getName()), self.LOG_DEBUG)
            self._log(
                "Language will not be able to use logfile-injection.", self.LOG_DEBUG)
        else:
            ...
            if (eval_code == None or eval_code.strip() == ""):
                self._log("XML-LD (%s) has no eval_kickstarter method defined." %
                          (self.getName()), self.LOG_DEBUG)
                self._log("Language will not be able to use logfile-injection." %
                          (self.getName()), self.LOG_DEBUG)
            self.eval_kickstarter = str(eval_code)

        write_node = getXMLNode(methods_node, "write_file")
        if (write_node == None):
            self._log("XML-LD (%s) has no write_file method defined." %
                      (self.getName()), self.LOG_DEBUG)
            self._log("Language will not be able to write files.",
                      self.LOG_DEBUG)
        else:
        ...
        if (len(self.detector_include) == 0):
            ...
            self._log(
                "  Only blindmode will work because they are used to retrieve informations out of the error message!", self.LOG_DEBUG)
        ...
        if (len(self.detector_readfile) == 0):
            ...
            self._log(
                "  No readfile bugs can be scanned if this is not defined.", self.LOG_DEBUG)
...
class fiPayload(baseTools):
    def __init__(self, xmlPayload, config, ParentName):
        ...
        self.source = str(getXMLNode(
            xmlPayload, "code").getAttribute("source"))
        self.ParentName = ParentName
        self._log("fimap PayloadObject loaded: %s" %
                  (self.name), self.LOG_DEVEL)
...
class fiExecMethod(baseTools):
    def __init__(self, xmlExecMethod, config):
        ...
        self._log("fimap ExecObject loaded: %s" %
                  (self.execname), self.LOG_DEVEL)
...
class fiFile(baseTools):
    def __init__(self, xmlFile, config):
        ...
        self._log("fimap FileObject loaded: %s" %
                  (self.filepath), self.LOG_DEVEL)
