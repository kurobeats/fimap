...
settings["dynamic_rfi"]["mode"] = "off" # Set to "ftp" or "local" to use Dynamic_RFI. Set it to "off" to disable it and rely on "filesrmt" files.
...
settings["dynamic_rfi"]["local"]["local_path"] = None # A non existing path on your filesystem without prefix which is reachable by http. Example: /var/www/payload
...
# Remote file inclusion test. You only need one URL.
settings["filesrmt"] = (
